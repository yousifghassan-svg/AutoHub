...
import type { VehicleBulkAction } from '@/lib/api/vehicle-api';
...
export default function VehiclesPage() {
  ...
  const query = useQuery({
    queryKey: ['admin', 'vehicles', listParams],
    queryFn: () => adminApi.vehicles.list(listParams),
  });

  const invalidate = () => void queryClient.invalidateQueries({ queryKey: ['admin', 'vehicles'] });

  const bulkMutation = useMutation({
    mutationFn: (input: { ids: string[]; action: VehicleBulkAction }) =>
      adminApi.vehicles.bulk(input),
    onSuccess: (_, vars) => {
      toast(`${vars.action} completed for ${vars.ids.length} vehicle(s)`, 'success');
      setSelected(new Set());
      invalidate();
    },
  ...
  const bulk = async (action: VehicleBulkAction) => {
    if (selected.size === 0) return;
    const ok = await confirm({
      title: `${action} ${selected.size} vehicle(s)?`,
      description: 'This action will apply to all selected vehicles.',
  ...
  const duplicateVehicle = async (id: string) => {
    const ok = await confirm({
      ...
      description: 'Create a copy of this vehicle as a new draft.',
      confirmLabel: 'Duplicate',
    });
    if (!ok) return;
    try {
      const copy = await adminApi.vehicles.duplicate(id);
      toast('Duplicate succeeded', 'success');
      invalidate();
      router.push(`/vehicles/${copy.id}`);
    } catch (e) {
  ...
  const exportCsv = async () => {
    try {
      const csv = await adminApi.vehicles.exportCsv({
        q: q || undefined,
        status,
        categoryCode: categoryCode || undefined,
        brandId: brandId || undefined,
      ...
      a.href = url;
      a.download = `autohub-vehicles-${Date.now()}.csv`;
      a.click();
  ...
  return (
    <div>
      ...
      <Card className="mb-4 space-y-4 p-4">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <Select
            label="Currency"
            value={currencyCode}
            onChange={(e) => setParam('currencyCode', e.target.value)}
            className="w-36"
          >
            <option value="">All</option>
      ...
      {query.isLoading ? (
        ...
        <ErrorState message="Failed to load vehicles" onRetry={() => void query.refetch()} />
                ...
                <tr key={item.id} className={item.deletedAt ? 'opacity-60' : undefined}>
                  ...
                  <Td>
                    <div className="space-y-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <TextLink href={`/vehicles/${item.id}`}>{listingTitle(item)}</TextLink>
                  ...
                  <Td>
                    <div className="flex flex-col gap-1 text-sm">
                      <Link href={`/vehicles/${item.id}`} className="text-brand hover:underline">
                        View
                      </Link>
                      <Link
                      ...
                      <button
                        ...
                        onClick={() => void duplicateVehicle(item.id)}
                      >
                        Duplicate
                      </button>
                    </div>
