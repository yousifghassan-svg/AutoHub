...
export default function VehicleReportsPage() {
  ...
  const query = useQuery({
    queryKey: ['admin', 'reports', 'vehicles', { page, status }],
    queryFn: () =>
      adminApi.reports.list({ page, pageSize: 20, status, domain: 'VEHICLE' }),
  });
  ...
  const setParam = (key: string, value: string) => {
    ...
    router.replace(`/vehicles/reports?${params.toString()}`);
  };
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Moderation reports for the vehicles marketplace."
      />
      ...
      {query.isLoading ? (
        ...
        <EmptyState title="No vehicle reports found" />
      ) : (
        <Card>
          <Table>
            <thead>
              ...
              {query.data!.items.map((report) => (
                <tr key={report.id}>
                  <Td>
                    {report.listing ? (
                      <Link
                        href={`/vehicles/${report.listing.id}`}
