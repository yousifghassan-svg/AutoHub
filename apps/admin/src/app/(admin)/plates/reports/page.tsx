...
export default function PlateReportsPage() {
  ...
  const query = useQuery({
    queryKey: ['admin', 'reports', 'plates', { page, status }],
    queryFn: () => adminApi.reports.list({ page, pageSize: 20, status, domain: 'PLATE' }),
  });
  ...
  const setParam = (key: string, value: string) => {
    ...
    router.replace(`/plates/reports?${params.toString()}`);
  };
  ...
  return (
    <div>
      <PageHeader
        title="Plate reports"
        description="Moderation reports for the plates marketplace."
      />
      ...
      {query.isLoading ? (
        ...
        <EmptyState title="No plate reports found" />
      ) : (
        <Card>
          <Table>
            <thead>
              ...
              {query.data!.items.map((report) => (
                <tr key={report.id}>
                  <Td>
                    {report.listing ? (
                      <Link
                        href={`/plates?highlight=${report.listing.id}`}
                  ...
                  <Td>
                    <div className="flex flex-wrap gap-1">
                      <Button size="sm" onClick={() => void act(report.id, 'resolve', 'Resolve')}>
                        Resolve
                      </Button>
                      <Button
                        size="sm"
