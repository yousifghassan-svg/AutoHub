...
export default function ListingDetailPage() {
  ...
  const query = useQuery({
    queryKey: ['admin', 'listings', params.id],
    queryFn: () => adminApi.listings.get(params.id, true),
  });
  ...
  const reports = useQuery({
    queryKey: ['admin', 'reports', 'listing', params.id],
  ...
  if (query.isError || !query.data) {
    return (
      <ErrorState
        message={query.error instanceof Error ? query.error.message : 'Listing not found'}
      />
    );
  }
  ...
  const plateGov =
    (Object.keys(GOVERNORATES) as IraqiGovernorate[]).find(
      (g) => GOVERNORATES[g].formatCode === plate?.formatCode,
    ) ?? 'Baghdad';
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="space-y-6">
            ...
            {media.length ? (
              ...
              <div className="rounded-xl bg-surface-muted p-6">
                <LicensePlate
                  ...
                  code={plate.regionCode || '11'}
                  letter={plate.series || 'A'}
                  number={plate.number || '00000'}
                  type={(plate.plateType as PlateType) || 'Private'}
                  framed
                  size="fill"
                />
              </div>
          ...
          <Card className="p-4">
            ...
            {audit.isLoading ? (
              ...
              <p className="text-sm text-ink-secondary">No audit events for this listing yet.</p>
            ) : (
        ...
        <div className="space-y-6">
          <Card className="space-y-3 p-4">
            ...
            <p className="text-2xl font-bold text-brand">{formatPrice(listing.primaryPrice)}</p>
          ...
          <Card className="space-y-3 p-4">
            ...
            {(reports.data?.items.length ?? 0) === 0 ? (
              <p className="text-sm text-ink-secondary">No reports for this listing.</p>
            ) : (
              <ul className="space-y-2 text-sm">
          ...
          <Card className="space-y-2 p-4">
            ...
            <div className="grid gap-2">
              <Button
                variant="secondary"
                onClick={() => void run('Publish', () => adminApi.listings.publish(listing.id))}
              >
                Publish
              </Button>
              <Button
                variant="secondary"
                onClick={() => void run('Unpublish', () => adminApi.listings.unpublish(listing.id))}
              >
                Unpublish
              </Button>
              <Button
                variant="secondary"
                onClick={() => void run('Reject', () => adminApi.listings.reject(listing.id))}
              >
                Reject
              </Button>
              <Button
                variant="secondary"
                onClick={() =>
                  void run('Duplicate', () => adminApi.listings.duplicate(listing.id), {
                    redirect: (result) => `/listings/${(result as AdminListing).id}`,
                  })
                }
              >
                Duplicate
              ...
              <Button
                variant="secondary"
                onClick={() =>
                  window.open(`${config.webUrl}/listings/${listing.id}`, '_blank', 'noopener,noreferrer')
                }
              >
                Preview listing
              </Button>
              <Button
                variant="secondary"
                onClick={() => void run('Approve', () => adminApi.listings.approve(listing.id))}
              >
                Approve / activate
              </Button>
              <Button
                variant="secondary"
                onClick={() => void run('Archive', () => adminApi.listings.archive(listing.id))}
              >
                Archive / deactivate
              </Button>
              <Button
                ...
                onClick={() =>
                  void run(
                    ...
                    () =>
                      listing.isFeatured
                        ? adminApi.listings.unfeature(listing.id)
                        : adminApi.listings.feature(listing.id),
                  )
                }
              >
                {listing.isFeatured ? 'Unfeature' : 'Feature'}
              ...
              {listing.deletedAt ? (
                <Button
                  variant="secondary"
                  onClick={() => void run('Restore', () => adminApi.listings.restore(listing.id))}
                >
                  Restore
                </Button>
              ) : (
                <Button
                  variant="danger"
                  onClick={() =>
                    void run('Soft delete', () => adminApi.listings.delete(listing.id), {
                      danger: true,
                    })
                  }
                >
              ...
              <Button
                ...
                onClick={() =>
                  void run(
                    'Permanent delete',
                    () => adminApi.listings.permanentDelete(listing.id, true),
                    { danger: true, redirect: '/listings' },
                  )
                }
              >
                Permanent delete
